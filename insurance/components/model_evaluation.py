...
# Defining Model Evaluation class
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)->artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            # which model is best trained current one or the model from saved model folder
            ...
            # previous trained model, transformer and encoder objects
            logging.info("Previous trained objects of transformer, model and target encoder")
            transformer = load_object(file_path=transformer_path)
            model = load_object(file_path=model_path)
            target_encoder = load_object(file_path=target_encoder_path)
            
            # Currently trained model, transformer and encoder objects
            logging.info("Currently trained model, transformer and encoder objects")
            current_transformer = load_object(file_path=self.data_transformation_artifact.transform_object_path)
            current_model  = load_object(file_path=self.model_trainer_artifact.model_path)
            current_target_encoder = load_object(file_path=self.data_transformation_artifact.target_encoder_path)
            

            # take test data for testing  
            ...
            print(f"Prediction using current trained model: {y_pred[:5]}")
